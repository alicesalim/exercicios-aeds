...
int poke_inserir_fim1(pokemon pokemon, poke_list *lista);
void inserir_fim(lista *list, char x []);
char* str_substring(const char *_Str, size_t _Start, size_t _End);
void poke_remover_inicio1(poke_list *lista);
...
int poke_inserir_fim1(pokemon pokemon, poke_list *lista){
    if(lista->n >= 5){
        /*caso seja maior que 5 remover inicio e colocar no fim*/
        poke_remover_inicio1(lista);
    }
    lista->array[lista->n] = pokemon;
    lista->n+=1;
    int total = 0;
    for(int i=0; i<lista->n; i++){
        total += atoi(lista->array[i].capture_rate);
    }
    lista->media = round((double)total / lista->n);
    return lista->media;
}
void poke_inserir_fim(pokemon pokemon, poke_list *lista){
    if(lista->n >= 5){
    ...
    int total = 0;
    for(int i=0; i<lista->n; i++){
        //passa lista->array[i] e chama imprimir
        total = total + atoi(lista->array[i].capture_rate);

    }
    lista->media= total/lista->n;
}
char* poke_remover_inicio(poke_list *lista){
    int i;
    if(lista->n == 0){
...
//ok
void p_operar(char* operacao, pokemon objeto, int pos, poke_list* lista){
    if(strcmp(operacao,"I")==0){
        int media = poke_inserir_fim1(objeto, lista);
        printf("Média: %d\n", media);
...
int main(){
   ...
   int i,j=0, count=0;
    ...
    foreach(poke , pokemons_salvos, count){
        int media = poke_inserir_fim1(*poke,&lista);
        printf("Média: %d\n", media);
    }
   ...
   for(i = 0; i < num_operacoes; i++) {
    ...
    // Executa a operação
    p_operar(operacao, pokemon, pos, &lista);
}
