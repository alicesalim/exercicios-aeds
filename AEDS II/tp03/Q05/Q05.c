...
#define IDNUBMERS_AND_MAXTAM_FILE 1000
...
void inse_fim(lista *list, char x []);
...
lista string_to_list(char elementos []){
    ...
    while(token!=NULL){
        ...
        inse_fim(&novaLista,trimmedToken);    //Insere o token na lista
...
//ok
void preencherlista(pokemon pokemons[]) {
...
//ok
void poke_(pokemon pokemon, poke_list *lista){
    int i;
...
//ok
void poke_inse_fim(pokemon pokemon, poke_list *lista){
    if(lista->n >= MAXTAM_LISTA){
        printf("Erro ao inserir!");
        exit(1);
    }
...
//ok
char* p_remover_inicio(poke_list *lista){
    int i;
    if(lista->n == 0){
...
//ok
char* p_remover_fim(poke_list *lista){
    if(lista->n == 0){
        printf("Erro ao remover!");
        exit(1);
    }
...
//ok
char* p_remover(int pos, poke_list *lista){
    int i;
