...
//Função: Permitem o acesso a atributos privados fora da classe sem modificar seu valor, garantindo o princípio de encapsulamento.
    ...
    public class Q07 {
        ...
        public static void insertionSort(Pokemon []ps){
            for(int i=0;i<ps.length;i++){
                for(int j=i;j>0;j--){
                    if(ps[j].getCaptureDate().compareTo(ps[j-1].getCaptureDate())<0){
                        Pokemon tmp = ps[j];
                        ps[j] = ps[j-1];
                        ps[j-1] = tmp;
                    }
        ...
        public static void main(String[] args) {
            ...
            Pokemon []ps = new Pokemon[pokemons.size()];
